...
SANITISED_IDF_PATH = $(PROJECT_PATH)/../esp-idf
...
all:
	@echo Result: $(BUILD_DIR_BASE)/$(PROJECT_NAME)-$(BOARD_NAME)_v$(HW_VER).$(HW_REV)-$(FW_VER).$(FW_REV).$(FW_BUILD)-$(shell git rev-parse --short HEAD).bin
...
ifndef CONFIG_SECURE_BOOT_BUILD_SIGNED_BINARIES
	...
	$(shell cp $(APP_BIN) $(BUILD_DIR_BASE)/$(PROJECT_NAME)-$(BOARD_NAME)_v$(HW_VER).$(HW_REV)-$(FW_VER).$(FW_REV).$(FW_BUILD)-$(shell git rev-parse --short HEAD).bin)
